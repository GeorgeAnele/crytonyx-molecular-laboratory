...
### 2.1 Medallion Layering Strategy
...
**Purpose:**  
Stores patient demographic attributes derived from laboratory test activity. Acts as a conformed dimension across clinical, billing, and logistics facts.
...
**Grain:** One row per unique payment method and payment status combination
...
| patient_sk | INT | Foreign key to `dim_patient` |
| test_sk | INT | Foreign key to `dim_test` |
| test_date_sk | INT | Foreign key to `dim_date` |
| location_sk | INT | Foreign key to `dim_location` |
...
| patient_sk | INT | Foreign key to `dim_patient` |
| invoice_date_sk | INT | Foreign key to `dim_date` |
| payment_sk | INT | Foreign key to `dim_payment` |
...
| patient_sk | INT | Foreign key to `dim_patient` |
| pickup_date_sk | INT | Foreign key to `dim_date` |
| courier_sk | INT | Foreign key to `dim_courier` |
| origin_location_sk | INT | Foreign key to `dim_location` |
...
- All Silver tables include:
  ...
  - `source_system`
  - `ingested_at`
- Silver data quality outcomes are logged in `silver.dq_log`
- Gold views expose only validated, conformed data suitable for analytics
...
- Ad-hoc analytical SQL querying
